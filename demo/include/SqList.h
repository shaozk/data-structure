...
// 附加元素操作
Status AppendSqList(SqList *L, ElemType e) {
...
// 修改
void UpdateSqList(SqList *L, int i, ElemType e) {
    if(i < 0 || i > L->length) printf("error\n");
    L->data[i-1] = e;
}
...
// 判断顺序表是否为空
Status IsSqEmpty(SqList *L) {
    return L->length == 0 ? TRUE : FALSE;
}

// 获取元素个数
int GetSqLength(SqList *L) {
    return L->length;
}
